...
class CodeEditor(QtWidgets.QPlainTextEdit):
    ...
    def resizeEvent(self, event):
        ...
        self.line_numbers_area.setGeometry(
            QtCore.QRect(cr.left(), cr.top(), self.line_numbers_area_width(), cr.height()))
        ...
        while block.isValid() and (top <= event.rect().bottom()):
            if block.isVisible() and (bottom >= event.rect().top()):
                ...
                if blockNumber == self.textCursor().block().blockNumber():
                    ...
                    painter.fillRect(0, top, self.line_numbers_area_width(), height,
                                     self.theme['editor_background'].lighter(140))
                else:
